...
public class Analyzer {
	...
	private volatile ArrayBlockingQueue<AudioBytesBlock> toAnalyze = new ArrayBlockingQueue<AudioBytesBlock>(10000);//ArrayList<AudioBytesBlock> toAnalyze = new ArrayList<AudioBytesBlock>();
	...
	private int sleepTime = 10; //in ms ; must be around 10 ms, otherwise the Graph on the GUI will start lagging
	...
	public Analyzer(Graph g) {
        ...
        /** Set the default color for each graph by index (usualy there are two graphs).<br>
         * The given Color will be used on generating the graphs while 
         * initializing the normalizer for the AudioDevice, usualy defined by the Constructor
         * 
         * @param channelIndex The index of the graph 
         * @param color The color to be set for the graph defined by the index
         **/
	public void setDefaultChannelGraphColor(int channelIndex, Color color){
		...
		return graphSettingsColor.get(new Integer(channelIndex));
         ...
         /** Set the default Y-offset for each graph by index (usualy there are two graphs).<br>
         * The given y-offset will be set on generating the graphs while 
         * initializing the normalizer for the AudioDevice, usualy defined by the Constructor
         * 
         * @param channelIndex The index of the graph 
         * @param color The y-offset to be set for the graph defined by the index
         **/
	public void setDefaultChannelGraphYOffset(int channelIndex, int yoffset){
		graphSettingsYOffset.put(new Integer(channelIndex), new Integer(yoffset));
	}
         ...
         *       5 : orange<br>
         * 
         * @param channelIndex the Graph defined by Index
         **/
	...
	public void clearGraphs(){
		synchronized (channelGraphs) {
			for (AudioGraph graph : channelGraphs) {
				g.removeGraph(graph);
			}
			channelGraphs.clear();
			normalizer = null;
			...
			initNormalizerThread = new Thread(new Runnable() {
				...
				public void run() {				
					...
					while (normalizer == null && initNormalizerActive){
						try{
							normalizer = new Normalizer(device.getSource().getFormat());
							channelsValueSum = new float[device.getFmt().getChannels()];
							chennalsDetailIndex = new int[device.getFmt().getChannels()];
...
//	}
	...
	private void initMergedChannelGraph(){
		synchronized (channelGraphs) {
			AudioGraph ag = new AudioGraph(new Color(226, 0, 116));
				...
				while (active) {
					...
					try{
						...
						for (int i = 0; i < samples; i++) {
							...
							if (channelGraphs.size() <= 0) break;

							try{
								
          						for (int j = 0; j < channels; j++) {
									
									//get current sample form the current channel ...
					...
					if (DEBUG) System.out.printf("Duraction: %9s ns  Buffer: %4s  Speed: %4s AudioByteBlocks/s\n", duractionTime, (10000 - toAnalyze.remainingCapacity()), speed);
				}
