...
public class LoLPlayerDB extends DataBase {
	...
	public HashMap<Integer, AudioFile> getPlaylistItems() throws SQLException {
		...
		HashMap<Integer, AudioFile> playlist = new HashMap<Integer, AudioFile>();
		
		while (rs.next()) {
			int no = rs.getInt("no");
			AudioFile af = new AudioFile(new File(rs.getString("path")), rs.getString("title"), rs.getString("author"));
			playlist.put(no, af);
		}
	...
	public void clearPlaylist(){
		String sql = String.format("DELETE FROM playlist;");
		try {
	...
	public void addPlaylistItem(AudioFile af) throws SQLException{
		int songid = addAudioFile(af);
		String sql = String.format("INSERT INTO playlist(songid) VALUES(%s);", songid);
		getConnection().sendSQLUpdate(sql);
	}
